...
X64_Emit :: struct {
	...
	// const_val[vN] holds the immediate when vN is an integer bc.BC_Const, plus a
...
slot_addr :: proc(vN: int) -> MemoryAddress {
	// [rbp - 8*(vN+1)]
	return AddressComponents{base = Register64.RBP, displacement = i32(-8 * (vN + 1))}
}

load :: proc(e: ^X64_Emit, reg: Register64, vN: bc.BC_Value) {
	mov_r64_m64(reg, slot_addr(int(vN)))
}

store :: proc(e: ^X64_Emit, vN: bc.BC_Value, reg: Register64) {
	mov_m64_r64(slot_addr(int(vN)), reg)
}
...
emit_body :: proc(e: ^X64_Emit) -> string {
	for inst in e.prog.insts {
		...
		case bc.BC_Load_Arg:
			...
			write([]u8{0x48, 0x8B, 0x00}) // mov rax, [rax]
...
emit_bin :: proc(e: ^X64_Emit, v: bc.BC_Bin) -> string {
	...
	// bc.Machine_Type may widen to i64 (an envelope without a canonical width), so we
	...
	case .Divide:
		if unsigned && b_const && !a_const {
			if sh, ok := log2_exact(e.const_val[int(v.b)]); ok {
				load(e, .RAX, v.a)
				shr_r64_imm8(.RAX, u8(sh)) // unsigned /2^k → shr
				store(e, v.dst, .RAX)
				return ""
			}
		}
	...
	case .Mod:
		cqo()
		idiv_r64(.RCX)
		mov_r64_r64(.RAX, .RDX) // remainder in rdx
	case .LShift:
		shl_r64_cl(.RAX) // count in CL (low byte of RCX)
	case .RShift:
		sar_r64_cl(.RAX) // arithmetic shift
	case:
...
emit_load_imm_rax :: proc(e: ^X64_Emit, v: i64) {
	if v >= 0 && v <= 0xFFFFFFFF {
		...
		u := u32(v)
		write([]u8{0xB8, u8(u), u8(u >> 8), u8(u >> 16), u8(u >> 24)})
		return
	}
	movabs_r64_imm64(.RAX, v)
}
...
emit_mul_const :: proc(e: ^X64_Emit, x: bc.BC_Value, k: i64) -> bool {
	switch {
	case k == 0:
		xor_r64_r64(.RAX, .RAX) // *0 → 0
		return true
	case k == 1:
	...
	if sh, ok := log2_exact(k); ok {
		load(e, .RAX, x)
		shl_r64_imm8(.RAX, u8(sh)) // *2^k → shl
		return true
	}
	...
	load(e, .RAX, x)
	if fits_imm32(k) {
		imul_r64_r64_imm32(.RAX, .RAX, u32(i32(k)))
		return true
	}
	return false
}
...
bc_const_operand :: proc(e: ^X64_Emit, a, b: bc.BC_Value) -> (k: i64, x: bc.BC_Value, ok: bool) {
