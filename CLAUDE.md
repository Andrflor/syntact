...
- **`test/typecheck`** — builds and runs (constraint-satisfaction + raw-cast `::` cases).
...
- **typecheck** — same JSON shape as analyze (`expect_errors`), focused on constraint-satisfaction cases (`(string|u8):b -> "hi"`, `~10:a -> 5`, …), including carve **implicit-constraint** cases (`u8:y -> x` then `a{x -> -11}` → `Constraint_Mismatch`, transitive and `x+y` variants) and **raw-cast** cases (`(a+b)::u8` satisfies `u8:c`; `??::u8`; `Invalid_Cast` for a layout-less target like `10..37`/`int`/`float`/disjoint union). *Builds and runs* (61 cases).
