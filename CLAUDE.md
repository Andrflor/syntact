...
**It is YOUR job to run the tests and report the raw results to the maintainer — the maintainer decides whether the outcome is acceptable, not you.** Run the relevant suite after a change and paste back what actually happened (pass/fail counts, the failing case names and messages). Do NOT pre-run a baseline before a change just to diff before/after, and do NOT silently judge a result as "fine" — surface it and let the maintainer validate.
...
- **parse.odin** — lexer + parser machinery. Table-driven lexer, single-pass Pratt parser covering the full grammar (bindings `->`/`<-`, productions, carving `{}`, extension `+{}`, collapse `!`, patterns `?`, constraints `:`, raw casts `::`, ranges `..`, events/resonance/reactivity, property `.`, externals `@`). `::` lexes as one `Cast` token and parses at `CONSTRAINT` precedence (tighter than `+`, so `(a+b)::u8` needs parens). **Unary precedence is operator-specific** (`parse_unary`): arithmetic negation `-` binds TIGHTER than range/shift (operand parsed at `CALL`), so `-5..5` is `(-5)..5`, not `-(5..5)`; the set operators `~`/`!` stay looser (operand at `UNARY`), so `~'a'..'z'` is `~('a'..'z')` — the negation of the whole range. **Bit-level operators**: `^`/`<<`/`>>` are direct (Xor/LShift/RShift); only `[&]`/`[|]`/`[~]` are bracketed, to stay visually distinct from the set-algebra `&`/`|`/`~`. All fold via interval arithmetic in integer.odin and lower via `op_to_bc`. **Equality is a single `=`** — `==` is NOT an operator: the lexer flags it `Bad_Double_Equal`, `parse_binary` raises an error and recovers it as one `=`. **The `.` token's kind is delimiter-sensitive** (`lex_dot`): a left delimiter before it → source-none property (`.x`, a self-mention into the carved scope). A `.` glued right after a pointing/event arrow (`z->.z`) is also source-none — `lex_dot` treats the arrow's closing `>` as a left delimiter ONLY in its two-byte `->`/`=>` form (byte before the `>` is `-`/`=`); a bare `>` (comparison) or execute-close (`Foo{...}<!>.bar`) keeps `.` as a member access.
...
- **analyze.odin** — semantic analysis. Builds a tree of `Scope_Type` (parallel arrays: `names`/`types`/`kind`/`values` + `type_folds`/`constraint_folds`). `walk()` dispatches on `Node_Kind` to per-kind `walk_<kind>` handlers. `sem_error`/`sem_warning` carry the node's `Span` + resolved position (the LSP reads the range directly). Carve specifics: `walk_property` resolves source-none properties against the carved scope; `recheck_carve` re-proves colored bindings after substitution (implicit-constraint mismatches). **Shorthand carve-of-a-carved-field**: a carve child `z{…}` whose source `z` names a DIRECT field of the scope being carved (`carve_shorthand_field`, via `self_resolve` — never the parent) is sugar for `z->.z{…}` — the field's `Reference` is threaded into the inner `walk_carve` as a `source_override`. A child whose source is a foreign scope (`a{data{6}}`, `data` defined elsewhere) is NOT a shorthand and stays a positional carve.
- **integer.odin / float.odin / string.odin / bool.odin** — the four domains. Each does interval arithmetic and the subset (constraint-satisfaction) check over its interval kind. `int_layout` maps a canonical interval to `{bits, signed}`; the raw-cast bit layer (`Bit_Repr`/`resize_bits`) lives in integer.odin. String is a `[]String_Interval` union with ordered `+` sequences, codepoint negation, and three-bound ranges. Bool is the finite set `{true,false}` with `&`/`|`/`~` as set ops. **Default value is PROPAGATED, not recomputed**: each domain leaf carries its `default_value`, and the integer fold (`fold_type_intervals`/`fold_constraint_intervals` return an `Integer_Type`, not bare intervals) threads it through `&`/`|` along the SOURCE order — `integer_type_union`/`integer_type_intersect` keep the LEFT operand's default when it still belongs to the folded set, else the right's, else the structural fallback `default_for_integer_intervals` (first finite bound; `0` for a fully-open set). The signed builtins `i8/i16/i32/i64/int` carry an EXPLICIT `0` default (`make_int_range_default` in `init_builtins`) so a bare `i8` defaults to 0, not −128; this propagates like any other. Bool models the same with `Bool_Domain` (`has_default`/`default`); float/string still recompute structurally.
...
Key conventions:
- **Builtin names are lowercase**: `int`/`float`/`string`/`char`/`bool`/`none` + the fixed-width `u8`/`i32`/`f64`/… A capitalized name (`Array`, `Circle`) is always a user scope, never a builtin. `char` is the single-codepoint string set (`'\x00'..'\U0010FFFF'`, ordinal) — NOT an int alias; `char:a -> 'A'` holds but `char:a -> 65` is a Constraint_Mismatch. A raw cast `N::char` reads N as a CODEPOINT number (`65::char` → `'A'`), distinct from `N::string` which transmutes bytes (`cast_target` routes an ordinal-string target to `Cast_Target_Kind.Char`).
...
- **Capture** `name(e)` / `(e)`: a field may carry a `(e)` capture (parsed into `Identifier_Data.capture`). It is a SECOND, INVISIBLE alias of the same binding — stored in the `Scope_Type.captures` column (parallel to `names`), `""` when absent. `.`/carve scan `names` only, so a capture is invisible to them; only the mention path (`walk_identifier` → `scope_resolve(..., allow_capture=true)`) sees it, after visible names and before the parent walk. `(e)` keeps its field positional and adds the alias; `name(e)` keeps the visible name and adds the alias.
- **Division by zero is domain-split.** Integer `/`,`%` lower to `idiv`, which traps on a 0 divisor (#DE → SIGFPE), so the fold (`integer.odin`) requires the DIVISOR to be statically bounded and exclude 0 (`b_lo>0` or `b_hi<0`), else `nil` → `Invalid_operator`. The DIVIDEND may be unbounded. Float `/` follows IEEE 754 — never traps (`x/0`→±∞, `0/0`→NaN, like Rust/Go/C/LLVM) — so `float.odin` NEVER rejects: a divisor straddling 0 yields the whole line `(-∞,+∞)`, otherwise the corner quotients via `ext_div`.
...
- **codegen** — the **end-to-end** suite (89 cases, all pass; multi-combo: each case runs many `(args → expect)` pairs). Each case (`source`+`args`+`expect`+`kind`) is checked against BOTH backends: the interpreter (the oracle) AND the native x64 (emit ELF → run via libc `popen` → compare exit-status/stdout). An interp/native divergence fails the case. Covers every domain + strength reductions, lea selection, 32-bit arithmetic, patterns, carve/collapse/refs/set-ops, strings, floats, integer/float division by a constant.
- **typecheck** — constraint-satisfaction (`expect_errors`): self-match, raw-cast `::`, composites, carve implicit-constraints, executes, references in `&`/`|`/`~`, patterns, string sequences/negation/tri-range, self-property carve refs (`z->.z{…}`) + the `z{…}` shorthand, negative-bound ranges, and proof-by-default (`u8:a;i8:b;u16:c->a+b` typechecks because bare a/b fold to their 0 defaults proving a+b=0, while `??::u8 + ??::i8` does not). 487 pass.
...
- **default** — `binding` + `expect` (the materialized default across domains/composites), incl. signed builtins → 0, source-order `&`/`|` propagation, and negative-bound ranges. All pass.
