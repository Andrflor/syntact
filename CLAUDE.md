...
- **typecheck** — same JSON shape as analyze (`expect_errors`), focused on constraint-satisfaction cases (`(string|u8):b -> "hi"`, `~10:a -> 5`, …), including carve **implicit-constraint** cases (`u8:y -> x` then `a{x -> -11}` → `Constraint_Mismatch`, transitive and `x+y` variants). *Builds and runs* (44 cases).
