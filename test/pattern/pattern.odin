...
// A pattern case is either an exhaustiveness/typecheck assertion (expect_errors)
// or a reduction assertion (expect = the stringified reduced value), or both. A
// case with neither asserts only that analysis runs without crashing.
Pattern_Test_Case :: struct {
	name:          string `json:"name"`,
	description:   string `json:"description"`,
	source:        string `json:"source"`,
	expect_errors: []string `json:"expect_errors"`,
	expect:        Maybe(string) `json:"expect"`,
}

load_pattern_test_file :: proc(rel: string) -> (Pattern_Test_Case, bool, string) {
	path := test_path(rel)
	data, err := os.read_entire_file(path, context.allocator)
	if err != nil do return {}, false, fmt.tprintf("Failed to read test file: %s", path)
	tc: Pattern_Test_Case
...
run_pattern_test :: proc(path: string, t: ^testing.T) {
	arena: vmem.Arena
	defer vmem.arena_destroy(&arena)
	context.allocator = vmem.arena_allocator(&arena)

	tc, ok, msg := load_pattern_test_file(path)
	if !ok {
	...
	compiler.analyze(cache)
	context.user_ptr = prev_user_ptr

	if !check_errors(tc, cache.analyze_errors[:], t) do return

	// Optional reduction assertion: reduce the root scope and compare the value.
	if expect, has := tc.expect.?; has {
...
// check_errors compares the analyzer's reported errors against expect_errors,
// in order. Returns false (and reports) on any mismatch.
check_errors :: proc(
	tc: Pattern_Test_Case,
	actual_errors: []compiler.Analyzer_Error,
	t: ^testing.T,
) -> bool {
	...
	for expected_str, i in tc.expect_errors {
		...
		if actual_errors[i].type != expected_type {
			testing.expectf(
				t,
				false,
